...
@property (nonatomic) SambaPlayer *player;
...
- (void)viewDidLoad {
    ...
    _player = [[SambaPlayer alloc] init];
                        ...
                        [[APIClient sharedInstance] loadRemoteImageFromURL:[NSURL URLWithString:urlString] andExecuteBlock:^(BOOL success, UIImage *image, NSURL *url) {
                            dispatch_async(dispatch_get_main_queue(), ^{
                                if (success) {
                                    ...
                                    [_collectionViewMovies reloadData];
                                } else {
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.urlMovie = @"http://techslides.com/demos/sample-videos/small.mp4";
...
//    [self performSegueWithIdentifier:@"MovieDetailSegue" sender:self];
    ...
    _player.view.frame = CGRectMake(0, 0, cell.frame.size.width, cell.frame.size.height);
//    SambaPlayer *player = [[SambaPlayer alloc] initWithParentViewController:self andParentView:cell];
    
    SambaMedia *media = [[SambaMedia alloc] init:cell.urlMovie];
    _player.media = media;
...
-(void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    if ([segue.identifier isEqualToString:@"MovieDetailSegue"]){
        NSArray *indexPathArray = [_collectionViewMovies indexPathsForSelectedItems];
